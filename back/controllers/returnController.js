...
const getReturnById = async (req, res) => {
  try {
    ...
    if (!returnInv) {
      return res.status(404).json({ message: 'المرتجع مش موجود' });
    }
...
const createReturn = async (req, res) => {
  try {
    ...
    if (!items?.length) {
      return res
        .status(400)
        .json({ message: 'لازم تضيف صنف واحد على الأقل' });
    }
    ...
    const totalAmount = recalcItems.reduce(
      (s, i) => s + i.total,
      0
    );
...
const updateReturn = async (req, res) => {
  try {
    ...
    if (returnInv.status === 'rejected') {
      return res
        .status(400)
        .json({ message: 'لا يمكن تعديل مرتجع مرفوض' });
    }
    ...
    if (wasApproved) {
      for (const oldItem of returnInv.items) {
        const oldTW =
          Number(oldItem.quantity) * Number(oldItem.weight);
    ...
    // ── حذف الأصناف القديمة ────────────────────────────────────────────────
    await prisma.returnInvoiceItem.deleteMany({
      where: {
    ...
    const updated = await prisma.returnInvoice.update({
      ...
      data: {
        ...
        supplierCode:
          supplierCode ?? returnInv.supplierCode,

        supplierName:
          supplierName ?? returnInv.supplierName,

        customerId:
          customerId ?? returnInv.customerId,

        supplierId:
          supplierId ?? returnInv.supplierId,
        ...
        originalInvoiceRef:
          originalInvoiceRef ??
          returnInv.originalInvoiceRef,

        refundMethod,

        refundCashAmount:
          Number(refundCashAmount) || 0,

        refundBankAmount:
          Number(refundBankAmount) || 0,
    ...
    // ── إعادة تطبيق أثر المخزن ─────────────────────────────────────────────
    if (wasApproved) {
      for (const newItem of updated.items) {
        const newTW =
          Number(newItem.quantity) * Number(newItem.weight);

        const mvType =
          updated.type === 'customer_return'
            ? 'return_in'
            : 'return_out';
        ...
        await createStockMovement({
          ...
          seasonId: updated.seasonId || null,
          createdById: req.user.id,
...
const approveReturn = async (req, res) => {
  try {
    ...
    if (returnInv.status === 'approved') {
      return res
        .status(400)
        .json({ message: 'المرتجع اتوافق عليه قبل كده' });
    }

    for (const retItem of returnInv.items) {
      const tw =
        Number(retItem.quantity) * Number(retItem.weight);

      const mvType =
        returnInv.type === 'customer_return'
          ? 'return_in'
          : 'return_out';
      ...
      await createStockMovement({
        ...
        warehouse: returnInv.warehouse,

        reference: returnInv.invoiceNumber,
    ...
    res.json({
      message: 'تم الموافقة على المرتجع ✅',
      returnInv: n(approved),
    });
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
