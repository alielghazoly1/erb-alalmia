...
// ✅ CRIT-NEW-002: approveReturn داخل prisma.$transaction(Serializable)
...
const getReturnById = async (req, res) => {
  try {
    const returnInv = await prisma.returnInvoice.findUnique({ where: { id: req.params.id }, include: invoiceIncludes() });
...
const getReturns = async (req, res) => {
  try {
    ...
    res.json({ returns: page.map(n), nextCursor, hasMore, total });
  } catch (err) { res.status(500).json({ message: err.message }); }
};
...
const createReturn = async (req, res) => {
  try {
    ...
    const normalizeId = (val) =>
      typeof val === 'object' ? (val?._id || val?.id || null) : (val || null);
...
const approveReturn = async (req, res) => {
  try {
    ...
    // ── Transaction: تحديث المخزون + تسجيل الحركات + الخزينة + الاعتماد ───────
    const approved = await prisma.$transaction(async (tx) => {
      for (const retItem of returnInv.items) {
        const tw     = extractTotalWeight(retItem);
        const mvType = returnInv.type === 'customer_return' ? 'return_in' : 'return_out';
        const delta  = returnInv.type === 'customer_return'
          ? { quantity:  safeNum(retItem.quantity), weight:  tw }
          : { quantity: -safeNum(retItem.quantity), weight: -tw };

        await updateStock(retItem.itemId, returnInv.warehouse, returnInv.seasonId, delta, tx);
      ...
      await recordReturn(returnInv, req.user, tx);
