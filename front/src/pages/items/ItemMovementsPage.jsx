...
// حركات الأصناف — فلاتر محفوظة في URL + طباعة in-page + lazy loading
...
const getInvoiceLink = (m) => {
  if (!m.referenceId) return null;
  if (m.type === 'sale'            || m.type === 'return_in')           return `/sales/${m.referenceId}`;
  if (m.type === 'purchase'        || m.type === 'return_out')          return `/purchase/${m.referenceId}`;
  if (m.type === 'transfer_in'     || m.type === 'transfer_out')        return `/transfers/${m.referenceId}`;
  if (m.type === 'manufacturing_in'|| m.type === 'manufacturing_out')   return `/manufacturing`;
  return null;
};
...
function printMovements({ item, movements, warehouse, startDate, endDate }) {
  ...
  const totalIn     = movements.filter(m => m.quantity > 0).reduce((s, m) => s + Math.abs(m.weight), 0);
  const totalOut    = movements.filter(m => m.quantity < 0).reduce((s, m) => s + Math.abs(m.weight), 0);
  const totalInQty  = movements.filter(m => m.quantity > 0).reduce((s, m) => s + Math.abs(m.quantity), 0);
  const totalOutQty = movements.filter(m => m.quantity < 0).reduce((s, m) => s + Math.abs(m.quantity), 0);
  ...
  const rows = movements.map((m, idx) => {
    ...
    return `
      <tr class="${idx % 2 === 0 ? 'even' : ''}">
        ...
        <td class="cen num" style="color:${color};font-weight:700">${tp.sign}${Math.abs(m.quantity).toLocaleString()}</td>
        <td class="cen num" style="color:${color}">${tp.sign}${fmt(Math.abs(m.weight))}</td>
        <td>${m.createdBy?.name || '—'}</td>
        <td class="cen">${link ? `<a href="${window.location.origin}${link}" target="_blank">فتح</a>` : '—'}</td>
      </tr>`;
  }).join('');
  ...
  const style = `
    <style>
      ...
      *{margin:0;padding:0;box-sizing:border-box}
      ...
      .meta{text-align:left;font-size:11px;color:#6b7280}
      .meta strong{display:block;font-size:14px;color:#1e40af;font-weight:700}
      ...
      .stat .lbl{font-size:10px;color:#6b7280;margin-bottom:2px}
      ...
      th{padding:7px 5px;text-align:center;font-weight:700;border:1px solid #1e3a8a}
  ...
  const html = `<!DOCTYPE html><html dir="rtl" lang="ar"><head><meta charset="UTF-8"/><title>كشف حركة — ${item.code}</title>${style}</head><body>
    <div class="header">
      <div>
    ...
    <div class="stats">
      <div class="stat in"><div class="lbl">إجمالي الوارد</div><div class="val">${totalInQty.toLocaleString()} كرتون</div><div style="font-size:11px;color:#16a34a">${fmt(totalIn)} ك</div></div>
      <div class="stat out"><div class="lbl">إجمالي الصادر</div><div class="val">${totalOutQty.toLocaleString()} كرتون</div><div style="font-size:11px;color:#dc2626">${fmt(totalOut)} ك</div></div>
      <div class="stat net"><div class="lbl">الصافي</div><div class="val">${(totalInQty - totalOutQty).toLocaleString()} كرتون</div><div style="font-size:11px;color:#1e40af">${fmt(totalIn - totalOut)} ك</div></div>
    </div>
    <table>
      <thead><tr><th>#</th><th>التاريخ</th><th>الوقت</th><th>النوع</th><th>المرجع</th><th>الموسم</th><th>المخزن</th><th>الكراتين</th><th>الوزن (ك)</th><th>بواسطة</th><th>فتح</th></tr></thead>
      <tbody>${rows}</tbody>
      <tfoot><tr>
        <td colspan="7" style="text-align:center">الإجمالي</td>
        <td class="cen">${(totalInQty - totalOutQty).toLocaleString()}</td>
        <td class="cen">${fmt(totalIn - totalOut)}</td>
        <td colspan="2"></td>
      </tr></tfoot>
    </table>
  ...
  // طباعة في نفس الصفحة بدون فتح نافذة جديدة
...
export default function ItemMovementsPage() {
  ...
  // ── استرجاع الفلاتر من الـ URL ─────────────────────────────────────────
  const initWarehouse = searchParams.get('warehouse') || 'october'; // default أكتوبر
  ...
  // بيانات الصنف المبدئي من الـ URL (لو جاي من صفحة الأصناف)
  ...
  // ── Auto-load لو فيه itemId في الـ URL ────────────────────────────────
  useEffect(() => {
    if (initItemId) {
      // جيب بيانات الصنف الكامله من الـ API عشان يكون فيه stock info
  ...
  const handleItemSelect = async (item) => {
    if (!item) {
      setSelectedItem(null); setMovements([]); setStockInfo(null); setTotal(0);
  ...
  const handleFilter = () => {
    if (selectedItem) loadMovements(selectedItem._id);
  };
    ...
    <div>
      ...
      <div className="mb-5 flex items-center justify-between flex-wrap gap-3">
        ...
        {selectedItem && movements.length > 0 && (
          <button
            onClick={() => printMovements({ item: selectedItem, movements, warehouse, startDate, endDate })}
        ...
        <div className="grid grid-cols-2 gap-4 mb-4">
          <div className="card bg-blue-50 border border-blue-200">
            ...
            <div className="flex gap-8">
              <div><p className="text-3xl font-black text-blue-700">{stockInfo.ramses.quantity.toLocaleString()}</p><p className="text-xs text-blue-400 mt-0.5">كرتون</p></div>
              <div><p className="text-3xl font-black text-blue-600">{fmt(stockInfo.ramses.weight)}</p><p className="text-xs text-blue-400 mt-0.5">كيلو</p></div>
            </div>
          </div>
          <div className="card bg-purple-50 border border-purple-200">
            <p className="text-xs text-purple-600 font-semibold mb-2">📦 مخزن أكتوبر — المخزون الحالي</p>
            <div className="flex gap-8">
              <div><p className="text-3xl font-black text-purple-700">{stockInfo.october.quantity.toLocaleString()}</p><p className="text-xs text-purple-400 mt-0.5">كرتون</p></div>
              <div><p className="text-3xl font-black text-purple-600">{fmt(stockInfo.october.weight)}</p><p className="text-xs text-purple-400 mt-0.5">كيلو</p></div>
            </div>
          </div>
        </div>
      )}
      ...
      {selectedItem && movements.length > 0 && (
        <div className="grid grid-cols-3 gap-4 mb-4">
          <div className="card text-center bg-green-50 border border-green-200">
            ...
            <p className="text-sm text-green-600">{fmt(totalIn)} ك</p>
          </div>
          <div className="card text-center bg-red-50 border border-red-200">
            ...
            <p className="text-sm text-red-600">{fmt(totalOut)} ك</p>
          </div>
          <div className="card text-center bg-blue-50 border border-blue-200">
            ...
            <p className="text-sm text-blue-600">{fmt(totalIn - totalOut)} ك</p>
          </div>
        </div>
      )}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gradient-to-l from-gray-50 to-blue-50/40 text-gray-600 border-b border-gray-200 text-xs">
                  ...
                  <th className="text-center px-3 py-3.5 font-semibold">الكراتين</th>
                  <th className="text-center px-3 py-3.5 font-semibold">الوزن (ك)</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {movements.map((m, idx) => {
                  ...
                  const invoiceLink = getInvoiceLink(m);
                  return (
                    <tr key={m._id} className="hover:bg-blue-50/30 transition-colors">
                      ...
                      <td className={`px-3 py-2.5 text-center font-bold text-sm ${isPositive ? 'text-green-600' : 'text-red-500'}`}>
                        {tp.sign}{Math.abs(m.quantity).toLocaleString()}
                      </td>
                      <td className={`px-3 py-2.5 text-center font-medium text-sm ${isPositive ? 'text-green-600' : 'text-red-500'}`}>
                        {tp.sign}{fmt(Math.abs(m.weight))}
                      </td>
                      <td className="px-3 py-2.5 text-gray-400 text-xs">{m.createdBy?.name || '—'}</td>
                      <td className="px-3 py-2.5 text-center">
                        {invoiceLink
                          ? <Link to={invoiceLink} target="_blank" className="text-xs bg-blue-50 text-blue-600 hover:bg-blue-100 px-2 py-1 rounded-lg font-medium transition-colors">فتح ←</Link>
