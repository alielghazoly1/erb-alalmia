// ─── components/LazySupplierStatementTable.jsx ───────────────────────────────
// جدول كشف حساب المورد بـ Lazy Loading
...
const TYPE_CFG = {
  invoice: { label: 'فاتورة شراء', bg: '#eff6ff', badge: '#2563eb', text: '#1e40af', link: (id) => `/purchase/${id}` },
  return:  { label: 'مرتجع',       bg: '#fff7ed', badge: '#ea580c', text: '#9a3412', link: (id) => `/returns/${id}`  },
  payment: { label: 'دفعة',         bg: '#f0fdf4', badge: '#16a34a', text: '#14532d', link: null                      },
};

const SupplierRow = memo(function SupplierRow({ row, idx, onEditPayment, backToStatement, sentinelRef }) {
  ...
  return (
    ...
    >
      ...
      <td style={{ padding: '7px 6px', textAlign: 'right', fontFamily: 'monospace', fontSize: '11px' }}>
        {link ? (
          <Link to={link} state={{ backTo: backToStatement, backLabel: 'كشف المورد' }}
      ...
      <td style={{ padding: '7px 6px', textAlign: 'center' }} className="print:hidden">
        {row.rowType === 'payment' && onEditPayment ? (
          ...
          <Link to={link} state={{ backTo: backToStatement, backLabel: 'كشف المورد' }} style={{ color: '#64748b', fontSize: '13px' }}>←</Link>
        ) : null}
      </td>
    </tr>
  );
...
export default function LazySupplierStatementTable({
  ...
  const totalDebit  = totals?.totalPurchases ?? rows.filter(r => r.rowType === 'invoice').reduce((s, r) => s + r.amount, 0);
